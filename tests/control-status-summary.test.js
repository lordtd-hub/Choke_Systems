...
try {
  ...
  assert.equal(summary.current_phase.position, 'third package implementation');
  assert.equal(summary.current_status.completed_layers.includes('spec core'), true);
  assert.equal(summary.current_status.in_progress.includes('third implementation package for the `Learning Record Store`'), true);
  assert.equal(summary.current_status.next_focus.includes('implement query services over repository ports'), true);
  assert.equal(summary.locked_task.task_id, 'P5-IMPL-003');
