...
try {
  ...
  assert.equal(summary.current_phase.position, 'adapter-rollout planning');
  assert.equal(summary.current_status.completed_layers.includes('spec core'), true);
  assert.equal(summary.current_status.in_progress.includes('adapter-rollout planning for the `Learning Record Store`'), true);
  assert.equal(summary.current_status.next_focus.includes('define the first adapter-swap rollout plan behind the current repository ports'), true);
  assert.equal(summary.locked_task.task_id, 'P5-PLAN-013');
