...
try {
  ...
  assert.equal(summary.current_status.in_progress.includes('canonical persistent state contract planning'), true);
  assert.equal(summary.current_status.next_focus.includes('define canonical state records and repository boundary for the `Learning Record Store`'), true);
  assert.equal(summary.locked_task.task_id, 'P5-PLAN-002');
